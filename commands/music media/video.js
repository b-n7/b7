  ...
  async execute(sock, m, args, PREFIX) {
    ...
    if (queryArgs.length > 0) {
      ...
      await sock.sendMessage(jid, { 
        text: `🎬 *YouTube Video Downloader*\n\n` +
              `💡 *Usage:*\n` +
              `• \`${PREFIX}video your search query\`\n` +
              `• \`${PREFIX}video YouTube URL\`\n` +
              `• \`${PREFIX}video query -doc\` - Send as document\n` +
              `• \`${PREFIX}video query -quality [360/480/720/1080]\`\n` +
              `📌 *Examples:*\n` +
              `• \`${PREFIX}video funny cats\`\n` +
              `• \`${PREFIX}video https://youtube.com/xxx\`\n` +
``
      }, { quoted: m });
      return;
    }

    console.log(`🎬 [VIDEO] Query: "${query}" | Quality: ${flags.quality}p`);

    try {
      // Send initial status
      const statusMsg = await sock.sendMessage(jid, { 
        text: `🔍 *Searching:* "${query}"\n` +
              `📹 *Quality:* ${flags.quality}p\n` +
              `⏳ *Please wait...*`
      }, { quoted: m });
      ...
      if (query.match(/(youtube\.com|youtu\.be)/i)) {
        ...
        try {
          ...
          await sock.sendMessage(jid, { 
            text: `❌ Search failed: ${searchError.message}`,
            edit: statusMsg.key 
          });
          return;
        }
      ...
      console.log(`🎬 [VIDEO] Found: "${videoTitle}" | URL: ${videoUrl}`);
      ...
      try {
        ...
        if (downloadResponse.data?.result) {
          ...
          console.log(`✅ [VIDEO] Keith API download link obtained`);
        } else {
        ...
        try {
          ...
          if (altResponse.data?.result?.video?.url) {
            ...
            console.log(`✅ [VIDEO] Alternative API download link obtained`);
          } else {
          ...
          await sock.sendMessage(jid, { 
            text: `❌ Video download failed from all sources.\n\nTry:\n1. Different video\n2. Lower quality (360/480)\n3. Direct YouTube URL`,
            edit: statusMsg.key 
          });
          return;
        }
      ...
      try {
        ...
        if (!contentType || !contentType.includes('video')) {
          console.warn(`⚠️ [VIDEO] Unexpected content type: ${contentType}`);
        }
        ...
        const contextInfo = {
          externalAdReply: {
            ...
            body: `📹 Video • ${fileSizeMB}MB • ${flags.quality}p`,
            mediaType: 1,
            sourceUrl: videoUrl,
        ...
        if (fs.existsSync(tempFile)) {
          fs.unlinkSync(tempFile);
          console.log(`🧹 [VIDEO] Cleaned temp file: ${tempFile}`);
        }

        await sock.sendMessage(jid, { 
          text: `✅ *Video Downloaded!*\n\n"${videoTitle}"\n📦 Size: ${fileSizeMB}MB\n🎬 Quality: ${flags.quality}p\n📤 Format: ${flags.doc ? 'Document' : 'Video Message'}\n🔧 API: ${apiUsed}`,
          edit: statusMsg.key 
        });

        console.log(`✅ [VIDEO] Success: "${videoTitle}" (${fileSizeMB}MB, ${flags.quality}p) via ${apiUsed}`);

      } catch (downloadError) {
        console.error("❌ [VIDEO] Download error:", downloadError.message);
        await sock.sendMessage(jid, { 
          text: `❌ Failed to process video: ${downloadError.message}`,
          edit: statusMsg.key 
        });
        if (fs.existsSync(tempFile)) {
