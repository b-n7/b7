...
async function getPm2AppName() {
  try {
    ...
    const firstAppLine = lines.find(line => line.includes('│') && !line.includes('App name'));
    if (firstAppLine) {
      const parts = firstAppLine.split('│').filter(p => p.trim());
...
export default {
  ...
  async execute(sock, m, args) {
    ...
    try {
      ...
      if (useZip) {
        await updateStatus('📥 **ZIP Update Selected**\nDownloading latest version...');
        updateResult = await updateViaZip();
      } else if (useGit) {
        await updateStatus('🌐 **Git Update Selected**\nPulling changes...');
        ...
        try {
          ...
          if (fs.existsSync(gitDir)) {
            await updateStatus('🌐 **Auto: Using Git Update**\nPulling changes...');
            updateResult = await updateViaGit();
          } else {
            throw new Error('No git repo');
          }
        } catch {
          await updateStatus('📥 **Auto: Using ZIP Update**\nDownloading latest version...');
          updateResult = await updateViaZip();
        }
      }
      ...
      if (!noDeps) {
        ...
        try {
          ...
          await updateStatus('✅ **Dependencies installed**');
        } catch (error) {
          console.warn('npm ci failed, trying npm install:', error.message);
          try {
            await run('npm install --no-audit --no-fund', 120000);
            await updateStatus('⚠️ **Dependencies installed with fallback**');
          } catch {
            await updateStatus('⚠️ **Could not install some dependencies**');
          }
        }
      }
      ...
      // Restart if needed
      if (noRestart) {
        await updateStatus('✅ **Update Complete!**\nRunning without restart.\nUse `.restart` to apply changes.');
        return;
      }
      
      await updateStatus('🔄 **Update successful! Restarting...**\nBot will be back in 5 seconds.');
